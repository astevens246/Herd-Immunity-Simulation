...
class Simulation:
    ...
    def _create_population(self):
        ...
        population.extend([Person() for _ in range(num_vaccinated)])
    ...
    def calculate_iteration_stats(self):
        ...
        # Add a check for 'is_alive' attribute
        num_dead = sum(getattr(person, 'is_alive', False) for person in self.population)
        ...
        return num_new_infections, num_dead, current_stats
        ...
        for person in self.population:
            if person.is_alive and not person.is_vaccinated and person.infection:
                for _ in range(100):
                    ...
                    print(f"Interaction: {person} -> {random_person}")
    ...
    def interaction(self, infected_person, random_person, new_infections):
        if (
            ...
            if random.random() < self.virus.repro_rate:
                ...
                print("New infection occurred!")
                print(f"Reproduction rate: {self.virus.repro_rate}")
                print(f"Infected person: {infected_person}")
